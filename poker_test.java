...
public class poker_test {
	...
	public static boolean twotwosame(int []arr) {
		...
		if((number == 2 || number == 3)&&(threesame(arr) == false)) {
			return true;
		}
		else {
			return false;
	...
	public static void main(String[] args) {
		...
		str2 = sc.nextLine();
		for(int i = 0; i < 5; i++) {
			arr1[i] = change(str1.charAt(3*i));
		}
		for(int i = 0; i < 5; i++) {
			arr2[i] = change(str2.charAt(3*i));
		}
		//数组arr进行由小到大排序
		Arrays.sort(arr1);
		Arrays.sort(arr2);
		
		grade1 = grade(straight(arr1), samefigure(str1), threesame(arr1), twotwosame(arr1), onetwosame(arr1));
		grade2 = grade(straight(arr2), samefigure(str2), threesame(arr2), twotwosame(arr2), onetwosame(arr2));
		if(grade1 > grade2) {
			System.out.println("Black wins");
		}
			...
			judgement(grade1, grade2, arr1, arr2);
		}
